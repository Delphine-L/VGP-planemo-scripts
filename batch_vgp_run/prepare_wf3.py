...
def main():
    ...
    parser = argparse.ArgumentParser(
                        ...
                        usage='prepare_wf3.py  -t  <Tracking table> -g <Galaxy Instance> -k <API Key> [OPTIONS]',
                        formatter_class=argparse.RawTextHelpFormatter,
                        epilog=textwrap.dedent('''
                                            General outputs: 
                                            - {Tracking_table}: The tracking table updated with wf3 runs. 
                                            For each species in {table}:
                                            - {assembly_id}/job_files/wf3_{assembly_id}_{suffix}.yaml: The yaml file with the job inputs and parameters.
                                            - {assembly_id}/invocations_json/wf3_invocation_{assembly_id}_{suffix}.json:  The json file with the invocation details.
                                            '''))
    ...
    use_id.add_argument('-i', '--wfl_id', dest="wfl_id", default="",   required=False,  default="", help="Workflow ID.")
    ...
    if args.from_id and args.from_file:
        ...
        if 'VGP3' not in wfl_name:
            raise SystemExit("Error: The workflow ID provided does not correspond to the Assembly-Hifi-only-VGP3 workflow. Please check the ID.")
    ...
    for i,row in infos.iterrows():
        ...
        list_histories.append(history_id)
        cmd_line="planemo run  "+worfklow_path+" "+yml_file+"--simultaneous_uploads --check_uploads_ok --engine external_galaxy --galaxy_url "+galaxy_instance+" --galaxy_user_key $MAINKEY --history_id "+history_id+" --no_wait --test_output_json "+res_file+" > "+log_file+" 2>&1  &"
