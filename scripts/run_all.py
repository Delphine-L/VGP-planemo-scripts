...
# Import get_urls from scripts directory
import sys
from pathlib import Path

# Get the directory containing this script (scripts/)
scripts_dir = Path(__file__).parent.resolve()
# Get the parent directory (VGP-planemo-scripts/)
repo_root = scripts_dir.parent

# Add both to Python path so we can import:
# - batch_vgp_run modules from repo root
# - get_urls from scripts directory
...
def main():
    ...
    if args.fetch_urls:
        ...
        for i, row in infos.iterrows():
            ...
            suffix = None
            if has_suffix:
                suffix_value = utils.normalize_suffix(row['Suffix'])
                if suffix_value:
                    suffix = suffix_value
        ...
        if 'Working_Assembly' not in infos.columns:
            def make_working_assembly(row):
                ...
                if 'Suffix' in row:
                    suffix = utils.normalize_suffix(row['Suffix'])
                    if suffix:
    ...
    profile_data['Metadata_directory']=utils.fix_directory(args.metadata_directory)
    ...
    suffix_run,galaxy_instance=utils.fix_parameters(args.suffix, profile_data['Galaxy_instance'])
    ...
    if args.resume or args.sync_metadata:
        ...
        galaxy_client.batch_update_metadata_from_histories(gi, list_metadata, profile_data, suffix_run, download_reports=args.download_reports)
